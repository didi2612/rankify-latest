...
export default function QRScannerPage() {
  ...
  // 🔹 Fetch participant when scannedData changes
  useEffect(() => {
    ...
    const fetchParticipant = async () => {
      ...
      try {
        const res = await fetch(
          `${SUPABASE_URL}/rest/v1/participants?name=eq.${encodeURIComponent(
            scannedData
          )}&select=id,name,project_title,institution,category`,
          {
            headers: {
              apikey: SUPABASE_API_KEY,
              Authorization: `Bearer ${SUPABASE_API_KEY}`,
  ...
  const handleSubmitScores = async () => {
    ...
    for (const field of scoreFields) {
      ...
      if (!field.value || isNaN(numValue) || numValue < 0 || numValue > 10) {
        toast.error(
          `Invalid score for ${field.key}. Must be a number between 0 and 10.`
        );
        return;
      }
    }
    ...
    try {
      ...
      if (existingScore.length > 0) {
        res = await fetch(
          `${SUPABASE_URL}/rest/v1/scores?id=eq.${existingScore[0].id}`,
          {
            method: "PATCH",
            headers: {
      ...
      if (res.ok) {
        ...
        toast.success(
          `Scores successfully ${action} for ${participant.name}! 🎉`
        );
        setScores({ innovation: "", impact: "", feasibility: "", comments: "" });
        setScannedData(null);
        setParticipant(null);
        ...
        toast.error(
          `Failed to submit scores. ${errorData.message || "Server error."}`
        );
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-screen bg-gray-950 text-white px-4 md:px-6 py-10 font-sans">
      <div className="max-w-xl mx-auto">
        ...
        <p className="text-gray-400 mb-8">
          Scan the participant's ID to begin scoring their project presentation.
        </p>
        ...
        >
          <h2 className="text-xl font-bold text-white mb-4 border-b border-gray-700/50 pb-3">
            QR Scanner
          </h2>
          ...
          <div className="mt-6 text-center w-full">
            {scannedData && (
              ...
              >
                ...
                <div className="flex items-center justify-center gap-3">
                  <p className="text-amber-400 font-mono break-all text-sm md:text-base">
                    {scannedData}
                  </p>
                  <button
                    onClick={handleCopy}
            ...
            >
              ...
              {participant && !loading && (
                <div className="bg-gray-900 p-6 rounded-xl border border-blue-500/30 shadow-2xl shadow-blue-500/10">
                  ...
                  <div className="grid gap-4 mb-6">
                    <ScoreInput
                      ...
                      onChange={(value) =>
                        setScores({ ...scores, innovation: value })
                      }
                    ...
                    <ScoreInput
                      ...
                      onChange={(value) =>
                        setScores({ ...scores, feasibility: value })
                      }
                  ...
                  <button
                    ...
                    disabled={
                      !scores.innovation || !scores.impact || !scores.feasibility
                    }
                  >
